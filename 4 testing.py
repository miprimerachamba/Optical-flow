...
def make_gradients_gaussian(video, sigma):
    gradients = np.zeros([3, dimensions[0], dimensions[1], dimensions[2]])
    gradients[0] = [ndi.gaussian_filter1d(video[i], sigma, order=1, axis=1) for i in range(0, dimensions[0])]
    gradients[1] = [ndi.gaussian_filter1d(video[i], sigma, order=1, axis=0) for i in range(0, dimensions[0])]
    gradients[2] = ndi.gaussian_filter1d(video, sigma, order=1, axis=0)  # for i in range(0,64)]
    return gradients
...
def plot_vectors(gradients, n, stride, gradient_type):
    path = "./" + video_name + "_n" + str(n) + "_stride" + str(stride) + "_gradient_" + gradient_type
    if not os.path.exists(path):
        os.mkdir(path)
    for frame in range(n, dimensions[0] - n):
        gradient_len = [(x ** 2 + y ** 2) ** (1 / 2) for x, y in
                        zip(gradients[0][frame].flatten(), gradients[1][frame].flatten())]
        cutoff = np.percentile(gradient_len, 95)
        plot = {"x": [], "y": [], "U": [], "V": []}
        print(frame)
        for y in range(n, dimensions[1] - n):
            for x in range(n, dimensions[2] - n):
                if (x % stride == 0 and y % stride == 0) and not (gradients[0][frame][y][x] ** 2 + gradients[1][frame][y][x] ** 2) ** (1 / 2) < cutoff:
        ...
        fig, ax = plt.subplots()
        fig.set_size_inches(gif_size, dimensions[1] / dimensions[2] * gif_size)
        ax.set_axis_off()
        fig.subplots_adjust(left=0, bottom=0, right=1, top=1)
        ax.imshow(vid_color[frame])
        ax.quiver(plot["x"], plot["y"], plot["U"], plot["V"], color="red", scale=20, width=0.001)
        if frame < 10:
        ...
        fig.savefig(path + "/frame" + frameno + ".png", dpi=400)
        plt.close()
    animate(path)
...
def LK(gradient, n, stride):
    if gradient == "gauss":
        vid_gradients = make_gradients_gaussian(vid, 2)
...
video_name = "dance"
print(os.listdir("./" + video_name))
vid_color = [ski.io.imread("./" + video_name + "/" + image) for image in os.listdir("./" + video_name)]
